...
class ConceptoRepositorio 
{
    ...
    function findByCompania($idCompania){
        ...
        try {
            ...
            $conn=OpenCon();            
            $stmt = $conn->prepare('select c.id, c.codigo, c.descripcion, c.observacion, c.usuario, c.fregistro,  c.compania, c.saldo, cu.codigo cuenta, cu.id idcuenta from '.$this->tabla.' c left join    '.$this->tablaCuenta.' cu on c.cuenta=cu.id  where c.compania=? ');
